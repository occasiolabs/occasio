...
function buildSummary(runId, receipt, mapping, framework) {
  ...
  return [
    ...
    ``,
    `> **Sharing note.** This bundle contains absolute file paths from the agent run`,
    `> (in \`chain.jsonl\` and \`bom.cyclonedx.json\`). It is intended for internal`,
    `> audit consumption. Before distributing publicly (PR description, blog post,`,
    `> issue tracker), review the contents and scrub any paths or identifiers that`,
    `> should not leave the producer's environment. \`receipt.json\` carries no paths`,
    `> and is the safer surface for public sharing.`,
    ``,
